...
func TestInstrument_NamesSpanAfterRouteAndRecordsRequestID(t *testing.T) {
	recorder := newRecordingProvider(t)

	handler := instrument("/policy/pdpo/v1/decision", http.HandlerFunc(
	...
	assert.Equal(t, "/policy/pdpo/v1/decision", spans[0].Name())
...
func TestInstrument_WithoutRequestIDHeader(t *testing.T) {
	...
	handler := instrument("/policy/pdpo/v1/statistics", http.HandlerFunc(
		func(res http.ResponseWriter, req *http.Request) { res.WriteHeader(http.StatusOK) }))
...
func TestInstrument_TracesUnauthorizedRequests(t *testing.T) {
	...
	handler := instrument("/policy/pdpo/v1/decision", basicAuth(http.HandlerFunc(
		func(res http.ResponseWriter, req *http.Request) { res.WriteHeader(http.StatusOK) })))
...
func TestInstrument_ContinuesIncomingTraceparent(t *testing.T) {
	...
	handler := instrument("/policy/pdpo/v1/data", http.HandlerFunc(
		func(res http.ResponseWriter, req *http.Request) { res.WriteHeader(http.StatusOK) }))
...
func TestRegisterHandlers_DecisionRouteIsTraced(t *testing.T) {
	...
	assert.Equal(t, "/policy/pdpo/v1/decision", muxRecorder.Ended()[0].Name())
}
