...
func TestSelfCheck_Success(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusOK)
		_ = json.NewEncoder(w).Encode(oapicodegen.HealthCheckReport{Healthy: boolPtr(true)})
	}))
	defer server.Close()
...
func TestSelfCheck_NotHealthy(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		_ = json.NewEncoder(w).Encode(oapicodegen.HealthCheckReport{Healthy: boolPtr(false)})
	}))
	defer server.Close()
