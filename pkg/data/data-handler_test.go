...
/*func TestPatchHandler_PolicyExists(t *testing.T) {
	...
	if err != nil {
		fmt.Println("error in currentDateTime")
         ...
         policymap.LastDeployedPolicies = `{"deployed_policies_dict": [{"data": ["existing.path"],"policy": ["valid-policy"],"policy-id": "valid-policy","policy-version": "1.0.0"}]}`
	...
	requestBody := oapicodegen.OPADataUpdateRequest{
		...
		Data:       &[]map[string]interface{}{{"op": "add", "test": "value"}},
	}
