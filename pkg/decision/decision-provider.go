...
var httpToResponseCode = map[int]oapicodegen.ErrorResponseResponseCode{
	400: oapicodegen.InvalidOperation,
	401: oapicodegen.Unauthorized,
	405: oapicodegen.InvalidOperation,
	500: oapicodegen.InternalError,
	422: oapicodegen.OpaUndefinedError,
}

//Get the error code
func getHttpStatusCode(err string) int {
	// Iterate over the map to find the corresponding status code based on containment
	for statusCode, errorCode := range httpToResponseCode {
		if strings.Contains(err, string(errorCode)) {
			return statusCode
		}
	}
	return 500 // Default to InternalServerError
}
...
func sendErrorResponse(msg string, res http.ResponseWriter, httpStatus int, policyName string) {
	log.Warnf("%s", msg)
	decisionExc := createDecisionExceptionResponse(http.StatusBadRequest, msg, policyName)
...
func handleOpaDecisionError(res http.ResponseWriter, err error, policyName string) {
	var httpStatus = getHttpStatusCode(err.Error())
	decisionExc := createDecisionExceptionResponse(httpStatus, err.Error(), policyName)
	metrics.IncrementTotalErrorCount()
	writeErrorJSONResponse(res, httpStatus, err.Error(), *decisionExc)
}
