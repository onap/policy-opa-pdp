...
func getDirName(policy model.ToscaPolicy) []string {
	...
	for key, _ := range policy.Properties.Data {
		...
		dirNames = append(dirNames, strings.ReplaceAll(consts.Data+"/node"+key, ".", "/"))
...
func verifyPolicyByBundleCreation(policy model.ToscaPolicy) error {
	// get directory name
	dirNames := []string{strings.ReplaceAll(consts.Data+"/node/"+policy.Name, ".", "/"), strings.ReplaceAll(consts.Policies+"/"+policy.Name, ".", "/")}
	// create bundle
